...
public class ApkDecoder {
    ...
    public void decode(File outDir) throws AndrolibException, IOException, DirectoryException {
        try {
            if (!get().forceDelete && outDir.exists()) {
            ...
            LOGGER.info("Using Apktool " + ApkBuilder.getVersion() + " on " + mApkFile.getName());

            decodeManifest(outDir);
            decodeResources(outDir);

            if (hasSources()) {
                switch (get().decodeSources) {
    ...
    public ResTable getResTable() throws AndrolibException {
        if (mResTable == null) {
            ...
            mResTable.setAnalysisMode(get().analysisMode);
        }
        return mResTable;
    }
    ...
    private void writeMetaFile(File outDir) throws AndrolibException {
        ...
        meta.version = ApkBuilder.getVersion();
        meta.apkFileName = mApkFile.getName();

        if (mResTable != null) {
            mResTable.initMetaInfo(meta, outDir);
            if (get().frameworkTag != null) {
                meta.usesFramework.tag = get().frameworkTag;
            }
        } else {
            if (mMinSdkVersion > 0) {
    ...
    private void decodeSourcesSmali(File outDir, String filename)
        ...
        try {
            ...
            DexFile dexFile = SmaliDecoder.decode(mApkFile, smaliDir, filename,
                    get().baksmaliDebugMode, get().apiLevel);
    ...
    private void decodeManifest(File outDir) throws AndrolibException {
        if (hasManifest()) {
            if (get().decodeResources == Config.DECODE_RESOURCES_FULL ||
                    get().forceDecodeManifest == Config.FORCE_DECODE_MANIFEST_FULL) {
                if (hasResources()) {
    ...
    private void copyRawFiles(File outDir)
        ...
        try {
            ...
            if (get().decodeAssets == Config.DECODE_ASSETS_FULL) {
