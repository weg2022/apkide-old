...
public class ApkBuilder {
    ...
    public void build(File outFile)
        ...
        get().isFramework = meta.isFrameworkApk;
        get().resourcesAreCompressed = meta.compressionType;
        get().doNotCompress = meta.doNotCompress;
        get().sync();
    ...
    public boolean buildSourcesRaw(File appDir, String filename)
        ...
        if (get().forceBuildAll || isModified(working, stored)) {
    ...
    public boolean buildSourcesSmali(File appDir, String folder, String filename)
        ...
        if (! get().forceBuildAll) {
            LOGGER.info("Checking whether sources has changed...");
        }
        if (get().forceBuildAll || isModified(smaliDir, dex)) {
            LOGGER.info("Smaling " + folder + " folder into " + filename + "...");
            //noinspection ResultOfMethodCallIgnored
            dex.delete();
            SmaliBuilder.build(smaliDir, dex, get().forceApi > 0 ? get().forceApi : mMinSdkVersion);
        }
        return true;
    }
    ...
    public boolean buildResourcesRaw(ExtFile appDir)
        ...
        try {
            ...
            if (get().forceBuildAll || isModified(newFiles(APK_RESOURCES_FILENAMES, appDir),
    ...
    public boolean buildResourcesFull(File appDir, UsesFramework usesFramework)
        ...
        try {
            ...
            if (get().forceBuildAll || isModified(newFiles(APP_RESOURCES_FILENAMES, appDir),
                    newFiles(APK_RESOURCES_FILENAMES, apkDir)) || (get().isAapt2() && !isFile(resourceFile))) {
                ...
                if (get().netSecConf) {
                    ...
                    if (meta.sdkInfo != null && meta.sdkInfo.get("targetSdkVersion") != null) {
                        if (Integer.parseInt(requireNonNull(meta.sdkInfo.get("targetSdkVersion"))) < ResConfigFlags.SDK_NOUGAT) {
    ...
    public boolean buildManifest(ExtFile appDir, UsesFramework usesFramework)
        ...
        try {
            ...
            if (get().forceBuildAll || isModified(newFiles(APK_MANIFEST_FILENAMES, appDir),
    ...
    private void copyUnknownFiles(File appDir, ZipOutputStream outputFile, Map<String, String> files)
        ...
        for (Map.Entry<String,String> unknownFileInfo : files.entrySet()) {
            ...
            LOGGER.info(String.format("Copying unknown file %s with method %d", unknownFileInfo.getKey(), method));
