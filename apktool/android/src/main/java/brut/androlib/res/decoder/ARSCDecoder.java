...
public class ARSCDecoder {
    ...
    private static final Logger LOGGER = Logger.getInstance(ARSCDecoder.class.getName());
    ...
    private ResConfigFlags readConfigFlags() throws IOException, AndrolibException {
        ...
        if (exceedingSize > 0) {
            ...
            if (exceedingBI.equals(BigInteger.ZERO)) {
                LOGGER.verbose("Config flags size > %d, but exceeding bytes are all zero, so it should be ok.",
                                KNOWN_CONFIG_BYTES);
            } else {
                LOGGER.warning("Config flags size > %d. Size = %d. Exceeding bytes: 0x%X.",
