...
public class SmaliBuilder {
    ...
    private final static Logger LOGGER = Logger.getInstance(SmaliBuilder.class.getName());
        ...
        if (fileName.endsWith(".smali")) {
            try {
                if (!SmaliMod.assembleSmaliFile(inFile, dexBuilder, mApiLevel, false, false)) {
                    throw new AndrolibException(getString(R.string.smalibuilder_could_not_smali_file) + fileName);
                }
            } catch (IOException | RecognitionException ex) {
                throw new AndrolibException(ex);
            }
        } else {
            LOGGER.warning(getString(R.string.smalibuilder_unknown_file_type) + inFile);
        }
        inStream.close();
    }
}
