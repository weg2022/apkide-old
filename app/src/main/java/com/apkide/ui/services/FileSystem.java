...
public class FileSystem {
    ...
    private static final ClassFileArchiveReader classFileArchiveReader = new ClassFileArchiveReader();
    ...
    public static Reader readFile(String filePath, String encoding) throws IOException {
        if (isNormalFile(filePath)) {
            if (classFileArchiveReader.isArchiveFileEntry(filePath)) {
                Reader reader = classFileArchiveReader.getArchiveEntryReader(new File(filePath).getParent(), FileUtils.getFileName(filePath), encoding);
                return new Reader() {
                    @Override
                    ...
                    public void close() throws IOException {
                        classFileArchiveReader.closeArchive();
                    }
                };
            }
    ...
    private static Reader readJarFileEntry(String filePath, String encoding) throws IOException {
        ...
        Reader reader = classFileArchiveReader.getArchiveEntryReader(jarFilepath, entryPath, encoding);
        return new Reader() {
            @Override
            public int read(char[] cbuf, int off, int len) throws IOException {
                return reader.read(cbuf, off, len);
    ...
    public static boolean isJarFileEntry(String path) {
        ...
        return jarPath != null && classFileArchiveReader.isArchiveFileEntry(path);
    }
    ...
    public static String resolvePath(String rootPath, String relativeOrAbsolutePath) {
        ...
        AppLog.d("Resolved (" + rootPath + ", " + relativeOrAbsolutePath + ") to " + res);
        return res;
    }
