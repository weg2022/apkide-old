...
public class Console extends View implements ModelListener {
    ...
    protected void configurePaint() {
        ...
        mySpaceCharWidth = myPaint.measureText(" ");
    ...
    public void setSelectionVisibility(boolean selectionVisibility) {
        if (mySelectionVisibility != selectionVisibility) {
            if (!selectionVisibility || computeDiffCount(
                    mySelectionAnchorLine, mySelectionAnchorColumn,
                    mySelectionPointLine, mySelectionPointColumn) != 0) {
    ...
    protected void updateSelection() {
        int count = computeDiffCount(mySelectionAnchorLine, mySelectionAnchorColumn,
                mySelectionPointLine, mySelectionPointColumn);
        if (count < 0) {
            updateSelection(mySelectionAnchorLine, mySelectionAnchorColumn,
                    mySelectionPointLine, mySelectionPointColumn);
        } else if (count > 0) {
            updateSelection(mySelectionPointLine, mySelectionPointColumn,
                    mySelectionAnchorLine, mySelectionAnchorColumn);
        } else {
    ...
    public Region getSelectedRegion() {
        return new Region(myFirstSelectedLine, myFirstSelectedColumn,
                myLastSelectedLine, myLastSelectedColumn);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(
                ...
                myModel == null ? 0 :
                        (int) (myModel.getLineCount() * myFontHeight));
    }
    ...
    protected float drawText(Canvas canvas, int line, int column, int length, float x, float y) {
        ...
        for (int i = column; i < length; i++) {
            char c = myModel.getChar(line, i);
            if (c == '\t') {
                myModel.drawLine(canvas, line, index, i - index, x, y, myPaint);
                x += nativeMeasureLine(line, index, i - index + 1);
                index = i + 1;
            }
        }

        if (index < length) {
            myModel.drawLine(canvas, line, index, length - index, x, y, myPaint);
            x += nativeMeasureLine(line, index, length - index);
        }
        return x;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (firstLine >= getModel().getLineCount())
            firstLine = getModel().getLineCount() - 1;

        if (lastLine >= getModel().getLineCount())
            lastLine = getModel().getLineCount() - 1;

        if (firstLine < 0)
            firstLine = 0;

        if (firstLine <= lastLine)
            redrawLines(canvas, firstLine, lastLine);
    }
    ...
    protected void redrawCaret(Canvas canvas, int line, int length, float x, float y) {
        if (myCaretColor != 0 && !mySelectionVisibility && myCaretLine == line) {
            ...
            if (showing) {
                ...
                if (myCaretColumn > 0)
                    caretX += measureLine(line, 0, myCaretColumn);
    ...
    protected void redrawText(Canvas canvas, int line, int length, float x, float y) {
        ...
        if (column < length) {
            ...
            x = drawText(canvas, line, column, length - column, x, y);
        }
