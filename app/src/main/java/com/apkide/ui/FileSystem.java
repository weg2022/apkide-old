...
import com.apkide.ui.util.JarArchiveReader;
...
public class FileSystem {
    ...
    private static final JarArchiveReader jarArchiveReader = new JarArchiveReader();
    ...
    public static Reader readFile(String filePath, String encoding) throws IOException {
        if (isNormalFile(filePath)) {
            if (jarArchiveReader.isArchiveFileEntry(filePath)) {
                Reader reader = jarArchiveReader.getArchiveEntryReader(new File(filePath).getParent(),getName(filePath), encoding);
                return new Reader() {
                    @Override
    ...
    public static String getEnclosingJar(String path) {
        if (isJar(path))
            return path;
    ...
    private static Reader readJarFileEntry(String filePath, String encoding) throws IOException {
        ...
        Reader reader = jarArchiveReader.getArchiveEntryReader(jarFilepath, entryPath, encoding);
        return new Reader() {
            @Override
            public int read(char[] cbuf, int off, int len) throws IOException {
                return reader.read(cbuf, off, len);
    ...
    public static String getJarFileEntryRelativePath(String path) {
    ...
    public static void writeFile(String filePath, Reader content) throws IOException {
        ...
        IOUtils.writeString(writer,readFully(content),true);
    }
    ...
    public static boolean isJarFileEntry(String path) {
        ...
        return jarPath != null && jarArchiveReader.isArchiveFileEntry(path);
    }
    ...
    public static List<String> getChildEntries(String dirPath) {
        if (isJarEntry(dirPath)) {
            ...
            try {
                entries = jarArchiveReader.getArchiveDirectoryEntries(jarFilepath, entryPath);
