...
public class MXSerializer implements XmlSerializer {
	...
	protected void ensureElementsCapacity() {
		...
		if (TRACE_SIZING) {
			AppLog.e(getClass().getName() + " elStackSize "
					+ elStackSize + " ==> " + newSize);
		}
	...
	protected void ensureNamespacesCapacity() { // int size) {
		...
		if (TRACE_SIZING) {
			AppLog.e(getClass().getName() + " namespaceSize " + namespacePrefix.length + " ==> " + newSize);
		}
		...
		for (int i = 0; i < value.length(); i++) {
			...
			if (ch == '<') {
				...
				if (ch == 13 || ch == 10 || ch == 9) {
					...
					if (TRACE_ESCAPING)
						AppLog.e(getClass().getName() + " DEBUG ATTR value.len=" + value.length()
								+ " " + printable(value));
			...
			if (ch == ']') {
				...
				if (ch == '&') {
					...
					if (ch == 9 || ch == 10 || ch == 13) {
						...
						if (TRACE_ESCAPING)
							AppLog.e(getClass().getName() + " DEBUG TEXT value.len=" + text.length()
									+ " " + printable(text));
			...
			if (ch == ']') {
				...
				if (ch == '&') {
					...
					if (ch == 9 || ch == 10 || ch == 13) {
						...
						if (TRACE_ESCAPING)
							AppLog.e(getClass().getName() + " DEBUG TEXT value.len=" + len + " "
