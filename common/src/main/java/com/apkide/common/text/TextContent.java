...
public class TextContent {
    ...
    private int gapLine = -1;    // the line on which the gap exists, the gap will always be associated with one line
    private static final int highWatermark = 300;
    private static final int lowWatermark = 50;

    private int[][] lines = new int[50][2];    // array of character positions and lengths representing the lines of text
    private int lineCount = 0;    // the number of lines of text
    private int expandExp = 1;    // the expansion exponent, used to increase the lines array exponentially
    private int replaceExpandExp = 1;    // the expansion exponent, used to increase the lines array exponentially
    ...
    public void addTextChangeListener(TextChangeListener listener) {
        if (listener == null) throw new IllegalArgumentException("listener can't null");
    ...
    public void replaceTextRange(int start, int replaceLength, String newText) {
        ...
        // inform listeners
        TextChangeEvent changeEvent = new TextChangeEvent();
        changeEvent.copyTo(event);
        for (TextChangeListener listener : textListeners) {
