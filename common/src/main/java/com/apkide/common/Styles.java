...
public class Styles {
    ...
    public void set(SyntaxKind[] styles, int[] startLines, int[] startColumns, int[] endLines, int[] endColumns, int size) {
        clear();
        for (int i = size - 1; i >= 0; i--) {
            if (styles[i] != null) {
                span((byte) styles[i].ordinal(), startLines[i], startColumns[i], endLines[i], endColumns[i]-1);
            }
        }
    }
