...
import com.apkide.java.decompiler.util.FastFixedSetFactory;
...
import com.apkide.java.decompiler.code.cfg.ExceptionRangeCFG;
import com.apkide.java.decompiler.main.DecompilerContext;
import com.apkide.java.decompiler.modules.decompiler.decompose.FastExtendedPostdominanceHelper;
import com.apkide.java.decompiler.modules.decompiler.deobfuscator.IrreducibleCFGDeobfuscator;
import com.apkide.java.decompiler.modules.decompiler.stats.BasicBlockStatement;
...
import com.apkide.java.decompiler.modules.decompiler.stats.DummyExitStatement;
import com.apkide.java.decompiler.modules.decompiler.stats.GeneralStatement;
...
import com.apkide.java.decompiler.modules.decompiler.stats.SynchronizedStatement;
...
public final class DomHelper {
  ...
  private static RootStatement graphToStatement(ControlFlowGraph graph) {
    ...
    if (stats.size() > 1 || firstblock.isSuccessor(firstblock)) { // multiple basic blocks or an infinite loop of one block
      ...
      firstst.addSuccessor(new StatEdge(StatEdge.EdgeType.BREAK, firstst, dummyexit, root));
    ...
    for (BasicBlock block : blocks) {
      ...
      for (BasicBlock succ : block.getSuccessors()) {
        ...
        StatEdge.EdgeType type;
        if (stsucc == firstst) {
          type = StatEdge.EdgeType.CONTINUE;
        }
        else if (graph.getFinallyExits().contains(block)) {
          type = StatEdge.EdgeType.FINALLY_EXIT;
          stsucc = dummyexit;
        }
        else if (succ.id == graph.getLast().id) {
          type = StatEdge.EdgeType.BREAK;
          stsucc = dummyexit;
        }
        else {
          type = StatEdge.EdgeType.REGULAR;
        }

        stat.addSuccessor(new StatEdge(type, stat, (type == StatEdge.EdgeType.CONTINUE) ? general : stsucc,
                                       (type == StatEdge.EdgeType.REGULAR) ? null : general));
      }
  ...
  public static VBStyleCollection<List<Integer>, Integer> calcPostDominators(Statement container) {
    ...
    do {
      ...
      for (Statement stat : lstStats) {
        ...
        List<Statement> lstSuccs = stat.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.FORWARD);
        ...
        if (!Objects.equals(domsSuccs, doms)) {
          ...
          List<Statement> lstPreds = stat.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.BACKWARD);
          ...
          if (current.isMonitorEnter()) {
            ...
            if (next.type == StatementType.CATCH_ALL) {
              ...
              if (ca.getFirst().isContainsMonitorExit() && ca.getHandler().isContainsMonitorExit()) {
                ...
                current.removeSuccessor(current.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL).get(0));

                for (StatEdge edge : current.getPredecessorEdges(StatEdge.EdgeType.DIRECT_ALL)) {
                  current.removePredecessor(edge);
                  edge.getSource().changeEdgeNode(StatEdge.EdgeDirection.FORWARD, edge, nextDirect);
                ...
                current.addSuccessor(new StatEdge(StatEdge.EdgeType.REGULAR, current, ca.getFirst()));
      ...
      for (Integer postId : posts) {
        ...
        while (true) {
          ...
          for (Statement handler : setHandlers) {
            ...
            if (!addhd) {
              List<Statement> hdsupp = handler.getNeighbours(StatEdge.EdgeType.EXCEPTION, StatEdge.EdgeDirection.BACKWARD);
              ...
              while (!lstStack.isEmpty()) {
                ...
                if (!(setNodes.contains(st) || (!same && st == post))) {
                  ...
                  if (st != head) {
                    ...
                    setPreds.addAll(st.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.BACKWARD));
                  }

                  // put successors on the stack
                  lstStack.addAll(st.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.FORWARD));

                  // exception edges
                  setHandlers.addAll(st.getNeighbours(StatEdge.EdgeType.EXCEPTION, StatEdge.EdgeDirection.FORWARD));
                }
              }
        ...
        boolean excok = true;
        for (Statement handler : setHandlers) {
          if (!handler.getNeighbours(StatEdge.EdgeType.EXCEPTION, StatEdge.EdgeDirection.BACKWARD).containsAll(setNodes)) {
            excok = false;
            break;
          }
        }
        ...
        if (excok) {
          ...
          if (setPreds.size() == 0) {
            if ((setNodes.size() > 1 ||
                 head.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.BACKWARD).contains(head))
  ...
  private static boolean checkSynchronizedCompleteness(Set<Statement> setNodes) {
    ...
    for (Statement stat : setNodes) {
      if (stat.isMonitorEnter()) {
        ...
        if (lstSuccs.size() != 1 || lstSuccs.get(0).getType() != StatEdge.EdgeType.REGULAR) {
          return false;
        }
