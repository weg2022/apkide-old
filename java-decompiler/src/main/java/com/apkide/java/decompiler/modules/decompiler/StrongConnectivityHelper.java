...
public class StrongConnectivityHelper {
  ...
  public StrongConnectivityHelper(@NonNull Statement startStatement) {
    ...
    for (Statement statement : startStatement.getStats()) {
      if (!setProcessed.contains(statement) && statement.getPredecessorEdges(StatEdge.EdgeType.DIRECT_ALL).isEmpty()) {
        visitTree(statement);
      }
    }
  ...
  private void visit(@NonNull Statement statement) {
    ...
    nextIndex++;
    List<Statement> successors = statement.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.FORWARD); // TODO: set?
